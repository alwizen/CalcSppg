...
<body class="bg-gradient-to-br from-orange-50 to-red-50 min-h-screen">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="text-center mb-8">
            <h1 class="text-4xl font-bold text-gray-800 mb-2">🍳 Kalkulator Dapur</h1>
        ...
        <div class="max-w-2xl mx-auto bg-white rounded-2xl shadow-lg p-8 mb-8">
            <form id="calculatorForm" class="space-y-6">
                    ...
                    <select name="recipe_id" id="recipe_id" required
                        ...
                        @foreach ($recipes as $recipe)
                            <option value="{{ $recipe->id }}" data-base-portions="{{ $recipe->base_portions }}">
                                {{ $recipe->name }} ({{ $recipe->base_portions }} porsi)
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="text-center">
                    <button type="submit"
                        class="bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white font-bold py-4 px-8 rounded-xl text-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200">
                        🧮 Hitung Bahan
                    </button>
                </div>
            </form>
        ...
        <div id="results" class="hidden max-w-4xl mx-auto bg-white rounded-2xl shadow-lg p-8">
            ...
            <div class="bg-blue-50 rounded-lg p-4 text-center">
                <p class="text-blue-800">
                    💡 <strong>Catatan:</strong> Hasil perhitungan ini berdasarkan resep standar.
                    Sesuaikan bumbu dan rasa sesuai selera Anda.
                </p>
            </div>
        </div>
        ...
        function displayResults(data) {
            ...
            recipeInfo.innerHTML = `
                <div class="grid md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                        <div class="text-2xl font-bold text-orange-600">${data.requested_portions}</div>
                        <div class="text-gray-600">Porsi Diminta</div>
                    </div>
                    <div>
                        <div class="text-lg font-semibold text-gray-800">${data.multiplier}x</div>
                        <div class="text-gray-600">Pengali (dari ${data.base_portions} porsi)</div>
                    </div>
                </div>
            `;
